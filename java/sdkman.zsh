# Initialize skdman dir and lazy load when related commands are first used.
export SDKMAN_DIR="$HOME/.sdkman"

# Define commands that will trigger the lazy load.
export JAVA_COMMANDS='java:javac:sdk'

# Defined function is required to complete the lazy load.
# - unset each java related command from the lazy load function.
# - unset the lazy load completion function
# - unset java commands variable
# - source init file
# - add autocompletion
# - unset autocompletion function
_sdkman_complete_lazy_load() {
  for cmd in ${(s/:/)JAVA_COMMANDS}; do
    unset -f $cmd 2> /dev/null
  done

  unset -f _sdkman_complete_lazy_load 2> /dev/null

  unset JAVA_COMMANDS

  [[ -s "${SDKMAN_DIR}/bin/sdkman-init.sh" ]] && source "${SDKMAN_DIR}/bin/sdkman-init.sh"

  _sdkman_add_autocomplete

  unset -f _sdkman_add_autocomplete 2> /dev/null
}

# Anonymous function to setup lazy load is executed on sourcing and then discarded.
function () {
  for cmd in ${(s/:/)JAVA_COMMANDS}; do
    $cmd() {
      _sdkman_complete_lazy_load
      "$0" "$@"
    }
  done
}

# Add sdkman autocompletion.
# https://github.com/ohmyzsh/ohmyzsh/blob/master/plugins/sdk/sdk.plugin.zsh
_sdkman_add_autocomplete() {
	case "${CURRENT}" in
	2)
		compadd -X $'Commands:\n' -- "${${(Mk)functions[@]:#__sdk_*}[@]#__sdk_}"
		compadd -n rm
		;;
	3)
		case "${words[2]}" in
		l|ls|list|i|install)
			compadd -X $'Candidates:\n' -- "${SDKMAN_CANDIDATES[@]}"
			;;
		ug|upgrade|h|home|c|current|u|use|d|default|rm|uninstall)
			compadd -X $'Installed Candidates:\n' -- "${${(u)${(f)$(find -L -- "${SDKMAN_CANDIDATES_DIR}" -mindepth 2 -maxdepth 2 -type d)}[@]:h}[@]:t}"
			;;
		e|env)
			compadd init
			;;
		offline)
			compadd enable disable
			;;
		selfupdate)
			compadd force
			;;
		flush)
			compadd archives broadcast temp version
			;;
		esac
		;;
	4)
		case "${words[2]}" in
		i|install)
			setopt localoptions kshglob
			if [[ "${words[3]}" == 'java' ]]; then
				compadd -X $'Installable Versions of java:\n' -- "${${${${${(f)$(__sdkman_list_versions "${words[3]}")}[@]:5:-4}[@]:#* | (local only|installed ) | *}[@]##* |            | }[@]%%+( )}"
			else
				compadd -X "Installable Versions of ${words[3]}:"$'\n' -- "${${(z)${(M)${(f)${$(__sdkman_list_versions "${words[3]}")//[*+>]+( )/-}}[@]:# *}[@]}[@]:#-*}"
			fi
			;;
		h|home|u|use|d|default|rm|uninstall)
			compadd -X "Installed Versions of ${words[3]}:"$'\n' -- "${${(f)$(find -L -- "${SDKMAN_CANDIDATES_DIR}/${words[3]}" -mindepth 1 -maxdepth 1 -type d -not -name 'current')}[@]:t}"
			;;
		esac
		;;
	5)
		case "${words[2]}" in
		i|install)
			_files -X "Path to Local Installation of ${words[3]} ${words[4]}:"$'\n' -/
			;;
		esac
		;;
	esac
}